 ...
 *  
 *  Merge sort is used to recursively find the solution. Inversions are grouped
 *  into left, right, and split inversions. Left (right) inversions are those
 *  contained in the left (right) array, split inversions are those with one 
 *  element in the left array and one in the right array. Demarcations are 
 *  inclusive.
 *
